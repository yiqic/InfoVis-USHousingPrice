...
function updateHeatMap(year, quarter, waitTimerDrawMap, waitTimerLoadData){
    ...
    setTimeout(function(){ 
       d3.map(completeDataSet, function(d) { return d.State; }).keys()
       .forEach(function(d){

            var filteredSet = completeDataSet.filter(function(da) { return da.State == d && da.YearQuarter == ("" + year + "Q" + quarter); })[0]; 
            var color = getColor(Number(filteredSet.AveragePrice.replace(/[^0-9\.]+/g,"")));

            sampleData[d]={average:filteredSet.AveragePrice, median:filteredSet.MedianPrice, color: color};        
        })
    }, waitTimerLoadData); 
    setTimeout(function(){
        //Remove Heat Map
...
function getColor(price) {
    //Middle Yellowish Color = #FFFFBF, Low Green #006837, and High Red = #A50026
...
// var stateList;
// var states = d3.map(completeDataSet, function(d) { return d.State; }).keys();
